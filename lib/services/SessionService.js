...
class SessionService {
    ...
    getUserById(userId, delegateeId) {
        return __awaiter(this, void 0, void 0, function* () {
            const userInRedis = yield this.redisClient.getObject(`user:${userId}del:${delegateeId}`);
            if (userInRedis) {
                return userInRedis;
            }
      ...
      `, userId);
                if (user) {
                    user.activeDelegations = yield t.any(`
          SELECT
            u.*,
      ...
      `, [user.role_id, user.user_id]);
                return Object.assign(Object.assign({}, user), { delegateeId,
                    permissions });
            }));
    ...
    createSession(userId, delegateeId, ua, ip) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    `, [sessionId, userId, delegateeId, ua, ip]);
            ...
            return { sessionId: session.session_id, jwt, user: yield this.getUserById(userId, delegateeId) };
        });
    }
    ...
    `, sessionId);
            ...
            const jwt = this.jwtService.sign({ userId, delegateeId });
            return { sessionId: session.id, jwt, user: yield this.getUserById(userId, delegateeId) };
        });
    }
